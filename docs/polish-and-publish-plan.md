...
2. Open a release-record PR that sets the package/lock version to `1.9.0-rc.4`, updates CHANGELOG,
...
5. Create immutable tag `v1.9.0-rc.4` only from the merged, verified release commit.
...
If further parent fixes are required, create rc.5; never move or replace rc.4.
...
3. The new candidate PR advances the lock from the merged rc.3 update (BCHC PR #6); verify its
...
1. After the candidate and live gates pass, open a stable release-record PR directly on top of the
   accepted rc.4 commit. Change `package.json`, `package-lock.json`, CHANGELOG, release status, and
   other version-bearing release documentation from `1.9.0-rc.4` to `1.9.0`; do not include a
...
6. Mark the rc.4 BCHC candidate PR superseded only after the stable update PR exists and its
