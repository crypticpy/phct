...
- **CI content pipeline.** Front-matter and file-size validation on every pull request, automatic thumbnail generation from uploaded PDFs, and workflows that scaffold cohort years, events and schedule updates from issues. Scaffolded content pull requests are labelled and title-prefixed at creation so code-review bots can be told to skip them — entries are data, not code; the admin guide documents the exact Sourcery dashboard setting.
